...
import { getCategoryTitleByValue, getTopStoriesData, getTotalTopStoriesCount } from '@/lib/sanity';
...
const CategroyNews = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const [data, count, categoryData] = await Promise.all([
                getTopStoriesData(currentPage, PAGE_SIZE, 'newsArticle', category),
                getTotalTopStoriesCount('newsArticle', category),
                                ...
                                {stories.length > 0 ? stories?.map((story: any, index: number) => (
                                    <div key={index} className="relative">
                                        ...
                                        <div className="bg-white bg-opacity-90 p-0 pt-4 lg:p-4 flex flex-col justify-end">
                                            ...
                                            <Link href={`${ROUTES.NEWS}${category}/${story.slug.current}`} className="text-lg font-semibold hover:underline">
                                                {story.title}
                                            </Link>
