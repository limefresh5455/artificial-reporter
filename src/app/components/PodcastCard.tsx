...
const PodcastCard = ({ podcast }: any) => {
    ...
    return (
        <div key={podcast._id} className="podcast-card bg-white rounded-lg border border-gray-200 p-5 flex flex-col md:flex-row items-start gap-4">
            <img src={podcast?.coverImage?.asset.url} className="w-40 rounded" alt="Podcast Cover" />
            <div className="flex-1">
                <a href="#" className="text-xl font-semibold text-[#000] hover:text-[#31795a]">{podcast.title}</a>
                <div className="  mt-2">
                    {podcast.audioFile ? (<AudioPlayer audioUrl={podcast.audioFile?.asset.url} />) : ''}
                </div>
            ...
            <div className=" flex flex-col  gap-1">
                <a href="#" className="apply-btn  px-5 py-1 rounded-2xl inline-flex items-center gap-2">Share <ExternalLink size={16} /></a>
